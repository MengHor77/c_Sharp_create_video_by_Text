...
namespace c_Sharp_video
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            RunFFmpeg($"-i \"{tempVideo}\" -vf \"subtitles='{srtEscaped}':force_style='FontName=Arial,FontSize=24,PrimaryColour=&H00FFFFFF,OutlineColour=&H00000000,BorderStyle=1,Outline=1,Shadow=0,Alignment=2,MarginL=10,MarginR=10,MarginV=30'\" -c:v libx264 -preset fast -crf 23 -c:a copy -movflags +faststart -y \"{outputVideo}\"");
        ...
        {
            try { using var image = System.Drawing.Image.FromFile(img); if (image.Width <= w && image.Height <= h) return img; } catch { }
            string scaled = Path.Combine(Path.GetTempPath(), $"scaled_{Guid.NewGuid():N}.jpg");
            RunFFmpeg($"-i \"{img}\" -vf \"scale={w}:{h}:force_original_aspect_ratio=decrease,pad={w}:{h}:(ow-iw)/2:(oh-ih)/2\" -y \"{scaled}\"");
            return scaled;
        }
